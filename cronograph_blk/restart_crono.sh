...
pkill -SIGTERM -o -x -f "^conky.*cronorc$" 2> /dev/null && {
	...
	pkill -SIGTERM -o -x -f "^conky.*cronorc$" 2> /dev/null && {
		pkill -SIGKILL -o -x -f "^conky.*cronorc$" 2> /dev/null && {
			notify-send "Cronograph Station BE" "Conky Cronograph Station BE cannot be restarted so it was killed." -i face-worried;
			$(${KillSnd}); exit 1;
		}
	}
	rm -f "$(dirname "$0")"/accuweather/*_cond;
	nice -n 5 conky -q -c "$(dirname "$0")"/cronorc && {
		sleep 5; notify-send "Cronograph Station BE" "Conky Cronograph Station BE was restarted." -i face-smile;
		$(${RestartSnd});
	}
} || {
